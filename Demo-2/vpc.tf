...
resource "aws_route" "route-2-clients-1" {
  route_table_id            = aws_route_table.f5_int.id
...
resource "aws_route" "route-2-clients-3" {
  route_table_id            = aws_route_table.public.id
  destination_cidr_block    = "10.1.0.0/16"
  vpc_peering_connection_id = aws_vpc_peering_connection.peering.id
}
...
################################################################################
# adding route back to F5 internal Interfaces for EKS communicating to Clients subnet
################################################################################



resource "aws_route" "route-2-clients" {
  route_table_id            = aws_route_table.eks.id
  destination_cidr_block    = "10.1.0.0/16"
  network_interface_id      = module.bigip02.interface_id
#  vpc_peering_connection_id = aws_vpc_peering_connection.peering.id
}